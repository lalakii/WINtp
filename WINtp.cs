...
public static class WINtp
{
    ...
    private static bool autoSyncTime = true;
    ...
    public static void Main()
    {
        ...
        if (File.Exists(cfgPath))
        {
            foreach (var item in File.ReadAllLines(cfgPath))
            {
                ...
                if (!string.IsNullOrEmpty(srv) && !srv.StartsWith("#") && !srv.StartsWith("/"))
                {
                    if (srv.Contains("="))
                    {
                        ...
                        else if (srv.Contains("autosynctime=false"))
                        {
                            autoSyncTime = false;
                        }
                    }
    ...
    private static void GetNtpTime(object server)
    {
        ...
        try
        {
            ...
            socket.SendTimeout = socket.ReceiveTimeout = 3000;
